    ...
    const fetchData = async () => {
      try {
        const response = await axios.get("/api", {
          headers: {
            path: `/discover/tv?include_adult=false&include_video=false&language=en-US&page=${page}&sort_by=popularity.desc&with_genres=${genreId}`,
          },
        });
        setData((val) =>
          val[0]
    ...
    <div className="flex-wrap flex flex-col gap-8 justify-between w-full py-10 px-10">
      <div>
        ...
        <a href="/#genres" className="hover:opacity-60">
          See all genres <ArrowForward className="scale-75" />
        </a>
      </div>
